...
#'@param sc scope to have been authorized by the user. Note that this scope alone will not return collaborative playlists, even though they are always private.
#' @examples \dontrun{
#' ## Example
#'  my_oauth <- spotifyOAuth(app_id="xxxx",client_id="yyyy",client_secret="zzzz")
#'  save(my_oauth, file="my_oauth")
#'  load("my_oauth")
#'  nacho <- getUser(user_id="nacho.989",token=my_oauth)
#' }
#'
#'
#'@export
...
spotifyOAuth<-function(app_id,client_id,client_secret, sc= "playlist-read-private user-top-read"){
...
return(httr::oauth2.0_token(spotifyR, myapp,scope = sc))}
